...
cv::Mat convertBGRToGray(cv::Mat img) {
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            float grayIntensity = 0.2126 * red + 0.7152 * green + 0.0722 * blue;
...
cv::Mat convertDXYToGradientLength(cv::Mat img) {
    ...
    int width = img.cols;
    int height = img.rows;
    cv::Mat gradImg(height, width, CV_32FC1); // создаем одноканальную картинку состоящую из 32-битных вещественных чисел
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            cv::Vec2f dxy = img.at<cv::Vec2f>(j, i);

            float grad = std::abs(std::sqrt(dxy[1]*dxy[1] + dxy[0]*dxy[0])); // взяли абсолютное значение производной по оси y
