...
void testBGRToGray() {
    cv::Mat img = cv::imread("C:\\Users\\vanar\\CLionProjects\\CPPExercises2021\\lesson05\\data\\valve.jpg");
    ...
    cv::imwrite("C:\\Users\\vanar\\CLionProjects\\CPPExercises2021\\lesson05\\resultsData\\valve_grey.jpg", gray);
}

void testSobel(const std::string &name) {
    cv::Mat img = cv::imread("C:\\Users\\vanar\\CLionProjects\\CPPExercises2021\\lesson05\\data\\" + name + ".jpg");
    rassert(!img.empty(), 23981920813);
    ...
    // TODO и удостоверьтесь что результат выглядит так как вы ожидаете, если нет - спросите меня
    cv::imwrite("C:\\Users\\vanar\\CLionProjects\\CPPExercises2021\\lesson05\\resultsData\\" + name + "_dx.jpg", dx);

    cv::Mat dy = convertDXYToDY(dxy); // TODO реализуйте функцию которая вытаскивает силу производной по y (ее абсолютное значение)
    // TODO и удостоверьтесь что результат выглядит так как вы ожидаете, если нет - спросите меня
    cv::imwrite("C:\\Users\\vanar\\CLionProjects\\CPPExercises2021\\lesson05\\resultsData\\" + name + "_dy.jpg", dy);
    ...
    // TODO и удостоверьтесь что результат выглядит так как вы ожидаете, если нет - спросите меня
    cv::imwrite("C:\\Users\\vanar\\CLionProjects\\CPPExercises2021\\lesson05\\resultsData\\" + name + "_gradientLength.jpg", gradientStrength);
