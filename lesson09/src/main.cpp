...
void test(std::string name) {
    std::string path = "C:\\Users\\vanar\\CLionProjects\\CPPExercises2021\\lesson09\\resultsData\\";
...
// TODO реализуйте фильтрацию прямых - нужно оставлять только те прямые, у кого много голосов (реализуйте функцию filterStrongLines(...) ):
    ...
    cv::imwrite(path + name + "_4_hough_circles.png", houghWithCircles);

    cv::Mat imgWithLines = drawLinesOnImage(img, lines);
    cv::imwrite(path + name + "_5_lines.png", imgWithLines);
}
